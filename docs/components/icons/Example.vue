<script setup>
  ...
  const mappedIconsBySize = Object.keys(icons).reduce((acc, current) => {
    ...
    acc[matches[0]] = acc[matches[0]] ? {...acc[matches[0]], ...{[current]: icons[current]}} : {[current]: icons[current]};
...
<template>
  <section v-for="size in Object.keys(mappedIconsBySize)" :key="size" style="background-color: var(--vp-c-bg-alt);" class="rounded-8 p-24 mb-24">
      ...
      <div class="grid gap-24 grid-cols-minmax-100px">
        <div v-for="(icon, fullName) in mappedIconsBySize[size]" :key="fullName">
          <div class="text-center">
            <div class="mx-auto mb-8 s-bg rounded-4 h-56 flex items-center justify-center flex-col">
